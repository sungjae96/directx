	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence3, "pol : 1351936", 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, "model num : 33", 20, 80, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, cpuString, 20, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence6, "1P", 1000, 20, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetMaxindex(int index, ID3D11DeviceContext* deviceContext)
{
	//char tempString[16];
	//char cpuString[16];
	//bool result;
	...
	//// Convert the cpu integer to string format.
	//_itoa_s(index, tempString, 10);

	//// Setup the cpu string.
	//strcpy_s(cpuString, "pol : ");
	////strcat_s(cpuString, tempString);
...
bool TextClass::PlayerInput(ID3D11DeviceContext* deviceContext)
{
	...
	{
		// Update the sentence vertex buffer with the new string information.
		result = UpdateSentence(m_sentence6, "1P", 1000, 20, 0.0f, 1.0f, 0.0f, deviceContext);
		if (!result)
		{
			return false;
		}
	...
	{
		// Update the sentence vertex buffer with the new string information.
		result = UpdateSentence(m_sentence6, "2P", 1000, 20, 0.0f, 1.0f, 1.0f, deviceContext);
		if (!result)
		{
			return false;
		}
...
bool TextClass::SetMousePosition(int mouseX, int mouseY, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence7, mouseString, 20, 220, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence8, mouseString, 20, 240, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
